...
// DeleteContraEPC deletes a ContraEPC item in the database
// 200 OK, 400 Bad Request, 500 Internal
func (inve *Inventory) DeleteContraEPC(ctx context.Context, writer http.ResponseWriter, request *http.Request) error {

	// Metrics
	metrics.GetOrRegisterGauge("Inventory.DeleteContraEPC.Attempt", nil).Update(1)

	startTime := time.Now()
	defer metrics.GetOrRegisterTimer("Inventory.DeleteContraEPC.Latency", nil).Update(time.Since(startTime))

	mDeleteLatency := metrics.GetOrRegisterTimer("Inventory.DeleteContraEPC.Delete-Latency", nil)
	mFindByEpcLatency := metrics.GetOrRegisterTimer("Inventory.DeleteContraEPC.FindByEpc-Latency", nil)

	mSuccess := metrics.GetOrRegisterGauge("Inventory.DeleteContraEPC.Success", nil)
	mDeleteErr := metrics.GetOrRegisterGauge("Inventory.DeleteContraEPC.Delete-Error", nil)
	mValidationErr := metrics.GetOrRegisterGauge("Inventory.DeleteContraEPC.Validation-Error", nil)
	mFindByEpcErr := metrics.GetOrRegisterGauge("Inventory.DeleteContraEPC.FindByEpc-Error", nil)

	copySession := inve.MasterDB.CopySession()
	defer copySession.Close()

	var mapping contraepc.DeleteContraEpcRequest
	...
	findTimer := time.Now()
	t, err := tag.FindByEpc(copySession, mapping.Epc)
	if err != nil {
		mFindByEpcErr.Update(1)
		return errors.Wrap(err, "Error checking database for existing Tag")
	}
	...
	// Make sure we only delete contra-epc tags
	if !contraepc.IsContraEpc(t) {
		return errors.Wrap(web.ErrInvalidInput, "specified epc is not a contra-epc")
	}

	deleteTimer := time.Now()
	if err := tag.Delete(copySession, t.Epc); err != nil {
		mDeleteErr.Update(1)
		return errors.Wrap(err, "Error deleting contra-epc")
	}
	mDeleteLatency.Update(time.Since(deleteTimer))

	// If we made it here, everything was a success
	mSuccess.Update(1)
	web.Respond(ctx, writer, nil, http.StatusNoContent)
	return nil
}

// CreateContraEPC creates a ContraEPC item in the database
// 200 OK, 400 Bad Request, 500 Internal
func (inve *Inventory) CreateContraEPC(ctx context.Context, writer http.ResponseWriter, request *http.Request) error {

	// Metrics
	metrics.GetOrRegisterGauge("Inventory.CreateContraEPC.Attempt", nil).Update(1)

	startTime := time.Now()
	defer metrics.GetOrRegisterTimer("Inventory.CreateContraEPC.Latency", nil).Update(time.Since(startTime))

	mReplaceLatency := metrics.GetOrRegisterTimer("Inventory.CreateContraEPC.Replace-Latency", nil)
	mSuccess := metrics.GetOrRegisterGauge("Inventory.CreateContraEPC.Success", nil)
	mValidationErr := metrics.GetOrRegisterGauge("Inventory.CreateContraEPC.Validation-Error", nil)
	mReplaceErr := metrics.GetOrRegisterGauge("Inventory.CreateContraEPC.Replace-Error", nil)

	copySession := inve.MasterDB.CopySession()
	defer copySession.Close()

	var mapping contraepc.CreateContraEpcRequest

	validationErrors, err := readAndValidateRequest(request, schemas.CreateContraEpcSchema, &mapping)
	...
	// NOTE: This fn is implemented as an all or nothing. If any 1 of the items are invalid, no items are inserted

	// The tag records to insert into the database
	tagData, err := generateContraEPC(mapping.Data, copySession)
	if err != nil {
		return err
	}

	replaceTimer := time.Now()
	if err := tag.Replace(copySession, &tagData); err != nil {
		mReplaceErr.Update(1)
		return errors.Wrap(err, "error adding contra-epc tags into database")
	}
	mReplaceLatency.Update(time.Since(replaceTimer))

	// If we made it here, everything was a success
	var results tag.Response
	results.Results = tagData
	mSuccess.Update(1)
